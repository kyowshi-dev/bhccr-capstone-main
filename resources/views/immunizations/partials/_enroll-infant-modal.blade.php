...
<div
    x-data='infantEnroll()'
    x-init="enrollOpen = {{ $enrollHasErrors ? 'true' : 'false' }}"
    @keydown.escape.window="enrollOpen = false"
    x-on:open-enroll-modal.window="enrollOpen = true; $nextTick(() => $refs.firstName?.focus())"
    @if ($enrollHasErrors)
        @php($duplicateMsg = $errors->first('duplicate'))
    @endif
>
    <template x-teleport="body">
        <div x-show="enrollOpen"
             ...
             role="dialog"
             aria-modal="true"
             aria-labelledby="enrollInfantTitle">
            <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="enrollOpen = false"></div>
            <div x-show="enrollOpen"
                 ...
                 class="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border shadow-lg"
                ...
                <form action="{{ route('immunizations.enroll-infant') }}" method="POST">
                    @csrf
                    <div class="sticky top-0 flex items-center justify-between border-b px-6 py-4" style="border-color: var(--border); background: var(--bg-surface);">
                        <div>
                            <h3 id="enrollInfantTitle" class="font-display font-semibold text-lg" style="color: var(--ink);">Enroll infant</h3>
                            <p class="text-xs mt-0.5" style="color: var(--ink-muted);">Quick-add an infant and attach them to a household.</p>
                        </div>
                        <button type="button" @click="enrollOpen = false" aria-label="Close" class="rounded-lg p-1.5 transition-colors hover:bg-black/5" style="color: var(--ink-muted);">
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <fieldset class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                <label for="enroll_suffix" class="mb-1 block text-xs font-medium" style="color: var(--ink-muted);">Suffix</label>
                                <input id="enroll_suffix" name="suffix" type="text" maxlength="50" value="{{ old('suffix') }}"
                                       class="w-full rounded-lg border px-3 py-2.5 text-sm focus:outline-none focus:ring-2"
                                       style="border-color: var(--border); color: var(--ink); --tw-ring-color: var(--accent-blue);">
                                @error('suffix')
                                    <p class="mt-1 text-xs font-medium" style="color: var(--danger);">{{ $message }}</p>
                    ...
                    <div class="sticky bottom-0 flex flex-wrap items-center justify-end gap-2 border-t px-6 py-4" style="border-color: var(--border); background: var(--bg-surface);">
                        <button type="button" @click="enrollOpen = false" class="rounded-lg border px-4 py-2.5 text-sm font-semibold transition-colors hover:bg-black/[0.03]" style="border-color: var(--border); color: var(--ink-muted);">
                            Cancel
                        </button>
