...
@section('content')
        <div class="space-y-4 lg:space-y-6">
            ...
            <form method="GET" action="{{ route('reports.family-planning') }}" id="familyPlanningFilterForm" class="space-y-4">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-left text-xs lg:text-sm">
                <thead class="bg-teal-soft border-b border-border">
                    <tr>
                        <th class="px-2 lg:px-4 py-2 lg:py-3 font-semibold text-ink">Method</th>
                        <th class="px-2 lg:px-4 py-2 lg:py-3 font-semibold text-ink w-24 text-right whitespace-nowrap">New Acceptors</th>
                        <th class="px-2 lg:px-4 py-2 lg:py-3 font-semibold text-ink w-28 text-right whitespace-nowrap">Continuing Users</th>
                        <th class="px-2 lg:px-4 py-2 lg:py-3 font-semibold text-ink w-24 text-right whitespace-nowrap">Drop Outs</th>
                        <th class="px-2 lg:px-4 py-2 lg:py-3 font-semibold text-ink w-24 text-right whitespace-nowrap">Visits</th>
                ...
                <tbody class="divide-y divide-border">
                    @forelse ($report['rows'] as $row)
                        <tr class="hover:bg-black/5">
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-ink">{{ $row->method }}</td>
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-right text-ink">{{ number_format($row->new_acceptors) }}</td>
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-right text-ink">{{ number_format($row->continuing_users) }}</td>
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-right text-ink">{{ number_format($row->drop_outs) }}</td>
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-right text-ink">{{ number_format($row->visits) }}</td>
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-right font-semibold text-ink">{{ number_format($row->total) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-6 lg:py-8 text-center text-ink-muted text-sm">No family planning clients for this period.</td>
                        </tr>
                    @endforelse
                </tbody>
                @if ($report['rows']->isNotEmpty())
                    <tfoot class="border-t border-border">
                        <tr class="bg-teal-soft">
                            ...
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-right font-semibold text-ink">{{ number_format($report['totalNew']) }}</td>
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-right font-semibold text-ink">{{ number_format($report['totalContinuing']) }}</td>
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-right font-semibold text-ink">{{ number_format($report['totalDropOuts']) }}</td>
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-right font-semibold text-ink">{{ number_format($report['totalVisits']) }}</td>
                            <td class="px-2 lg:px-4 py-2 lg:py-3 text-right font-semibold text-ink">{{ number_format($report['totalClients']) }}</td>
                        </tr>
                    </tfoot>
                @endif
            </table>
...
<script>
(function () {
    const form = document.getElementById('familyPlanningFilterForm');
    if (!form) return;
...
function downloadPdf() {
    const url = new URL('{{ route("reports.family-planning.download") }}', window.location.origin);
